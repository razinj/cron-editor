...
export class CronEditorComponent implements OnInit, OnChanges {
  ...
  currentState: number;
  defaultCustomWidth: string = '900px';

  get customWidth() {
    return {
      'width': this.options?.customWidth ? this.options?.customWidth : this.defaultCustomWidth
    }
  }
  ...
  public regenerateCron() {
    ...
    switch (this.activeTab) {
      case 'minutes':
        ...
        if (!this.options.removeSeconds) this.cron = `${this.state.minutes.seconds} ${this.cron}`;
        if (!this.options.removeYears) this.cron = `${this.cron} *`;
        break;
      case 'hourly':
        this.cron = `${this.state.hourly.minutes} 0/${this.state.hourly.hours} 1/1 * ?`;
        if (!this.options.removeSeconds) this.cron = `${this.state.hourly.seconds} ${this.cron}`;
        if (!this.options.removeYears) this.cron = `${this.cron} *`;
        break;
      case 'daily':
        switch (this.state.daily.subTab) {
          case 'everyDays':
            ...
            if (!this.options.removeSeconds) this.cron = `${this.state.daily.everyDays.seconds} ${this.cron}`;
            if (!this.options.removeYears) this.cron = `${this.cron} *`;
            break;
          case 'everyWeekDay':
            // tslint:disable-next-line:max-line-length
            this.cron = `${this.state.daily.everyWeekDay.minutes} ${this.state.daily.everyWeekDay.hours} ? * MON-FRI`;
            if (!this.options.removeSeconds) this.cron = `${this.state.daily.everyWeekDay.seconds} ${this.cron}`;
            if (!this.options.removeYears) this.cron = `${this.cron} *`;
            break;
          default:
        ...
        break;
      case 'weekly':
        const days = this.selectOptions.days.reduce((acc, day) => this.state.weekly[day] ? acc.concat([day]) : acc, []).join(',');
        this.cron = `${this.state.weekly.minutes} ${this.state.weekly.hours} ? * ${days}`;
        if (!this.options.removeSeconds) this.cron = `${this.state.weekly.seconds} ${this.cron}`;
        if (!this.options.removeYears) this.cron = `${this.cron} *`;
        break;
      case 'monthly':
        switch (this.state.monthly.subTab) {
          case 'specificDay':
            ...
            if (!this.options.removeSeconds) this.cron = `${this.state.monthly.specificDay.seconds} ${this.cron}`;
            if (!this.options.removeYears) this.cron = `${this.cron} *`;
            break;
          case 'specificWeekDay':
            ...
            if (!this.options.removeSeconds) this.cron = `${this.state.monthly.specificWeekDay.seconds} ${this.cron}`;
            if (!this.options.removeYears) this.cron = `${this.cron} *`;
            break;
          default:
      ...
      case 'yearly':
        switch (this.state.yearly.subTab) {
          case 'specificMonthDay':
            ...
            // tslint:disable-next-line:max-line-length
            this.cron = `${this.state.yearly.specificMonthDay.minutes} ${this.state.yearly.specificMonthDay.hours} ${day} ${this.state.yearly.specificMonthDay.month} ?`;
            if (!this.options.removeSeconds) this.cron = `${this.state.yearly.specificMonthDay.seconds} ${this.cron}`;
            if (!this.options.removeYears) this.cron = `${this.cron} *`;
            break;
          case 'specificMonthWeek':
            // tslint:disable-next-line:max-line-length
            this.cron = `${this.state.yearly.specificMonthWeek.minutes} ${this.state.yearly.specificMonthWeek.hours} ? ${this.state.yearly.specificMonthWeek.month} ${this.state.yearly.specificMonthWeek.day}${this.state.yearly.specificMonthWeek.monthWeek}`;
            if (!this.options.removeSeconds) this.cron = `${this.state.yearly.specificMonthWeek.seconds} ${this.cron}`;
            if (!this.options.removeYears) this.cron = `${this.cron} *`;
            break;
          default:
  ...
  private handleModelChange(cron: string) {
    ...
    let cronSeven = cron;
    if (this.options.removeSeconds) cronSeven = `0 ${cron}`;

    if (this.options.removeYears) cronSeven = `${cronSeven} *`;
  ...
  private validate(cron: string): void {
    ...
    if (!this.options.removeSeconds) expected++;

    if (!this.options.removeYears) expected++;
  ...
  private getDefaultAdvancedCronExpression(): string {
    if (this.options.removeSeconds && !this.options.removeYears) return '15 10 L-2 * ? 2001';
    if (!this.options.removeSeconds && this.options.removeYears) return '0 15 10 L-2 * ?';
    if (this.options.removeSeconds && this.options.removeYears) return '15 10 L-2 * ?';

    return '0 15 10 L-2 * ? 2001';
  }
